  ...
  return {
    update({ time, x, y, radius, hue, width, height }) {
      ...
      echoes.forEach((echo) => {
        ...
        echo.radius = echo.startRadius * (1 + progress * .5);
        echo.alpha = echo.startAlpha * Math.pow(1 - progress, 1.35);
      });
      ...
      if (time >= nextEmissionAt) {
        ...
        echoes.push({
          ...
          vy,
          distance: 0,
          exitDistance: distanceUntilExit({ x, y, radius: radius * 1.5, vx, vy, width, height }),
        });
        schedule(time);
      }
    },

...
export function createPacmanVisualization({
  ...
  function resetTraveler(traveler, width, height, initial = false) {
    ...
    traveler.kind = random() < .38 ? 'pacman' : 'ghost';
  ...
  return {
    render({ ctx, width, height, bands, frame }) {
      ...
      travelers.forEach((traveler, index) => {
        ...
        const image = traveler.kind === 'pacman'
          ? pacmanFrames[Math.floor(frame / Math.max(3, 7 - Math.min(3, Math.floor(bands.level * 4)))) % pacmanFrames.length]
          : ghosts[traveler.sprite];
        if (!image.complete || image.naturalWidth === 0) return;

        ctx.save();
        ctx.globalAlpha = traveler.alpha;
        ctx.translate(traveler.x, traveler.y + bob);
        ...
        if (traveler.kind === 'pacman' && index % 2 === 0) {
          ...
          ctx.arc(traveler.x - traveler.direction * size * .78, traveler.y + bob, Math.max(1.5, size * .035), 0, Math.PI * 2);
          ctx.fill();
        }
      });
    },
